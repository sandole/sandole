...
ICON_PR_MERGED = "M5.45 5.154A4.25 4.25 0 0 0 9.25 7.5h1.378a2.251 2.251 0 1 1 0 1.5H9.25A5.734 5.734 0 0 1 5 7.123v3.505a2.25 2.25 0 1 1-1.5 0V5.372a2.25 2.25 0 1 1 1.95-.218ZM4.25 13.5a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Zm8.5-4.5a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5ZM5 3.25a.75.75 0 1 0-1.5 0 .75.75 0 0 0 1.5 0Z"
ICON_PR_OPEN = "M1.5 3.25a2.25 2.25 0 1 1 3 2.122v5.256a2.251 2.251 0 1 1-1.5 0V5.372A2.25 2.25 0 0 1 1.5 3.25Zm5.677-.177L9.573.677A.25.25 0 0 1 10 .854V2.5h1A2.5 2.5 0 0 1 13.5 5v5.628a2.251 2.251 0 1 1-1.5 0V5a1 1 0 0 0-1-1h-1v1.646a.25.25 0 0 1-.427.177L7.177 3.427a.25.25 0 0 1 0-.354ZM3.75 2.5a.75.75 0 1 0 0 1.5.75.75 0 0 0 0-1.5Zm0 9.5a.75.75 0 1 0 0 1.5.75.75 0 0 0 0-1.5Zm8.25.75a.75.75 0 1 0 1.5 0 .75.75 0 0 0-1.5 0Z"
...
def icon(x: int, y: int, path: str, fill: str) -> str:
    return f'  <g transform="translate({x},{y})" fill="{fill}"><path d="{path}"/></g>'
...
def generate_svg(data: dict, prs: dict) -> str:
    ...
    if merged > 0:
        ...
        badges += icon(badge_x + 6, 77, ICON_PR_MERGED, "#ffffff") + "\n"
        badges += f'  <text x="{badge_x + 24}" y="89" font-family="{FONT}" font-size="11" font-weight="600" fill="#ffffff">{label}</text>\n'
        badge_x += w + 8
    if opened > 0:
        label = f"{opened} open"
        w = len(label) * 7 + 16
        badges += f'  <rect x="{badge_x}" y="74" width="{w}" height="22" rx="11" fill="#1f6feb"/>\n'
        badges += icon(badge_x + 6, 77, ICON_PR_OPEN, "#ffffff") + "\n"
        badges += f'  <text x="{badge_x + 24}" y="89" font-family="{FONT}" font-size="11" font-weight="600" fill="#ffffff">{label}</text>\n'
...
{icon(20, 20, ICON_REPO, "#8b949e")}
  <text x="42" y="33" font-family="{FONT}" font-size="14" font-weight="600" fill="#58a6ff">{name}</text>
  <text x="20" y="58" font-family="{FONT}" font-size="12" fill="#8b949e">{desc}</text>
{badges}  <circle cx="28" cy="124" r="6" fill="{lc}"/>
  <text x="40" y="128" font-family="{FONT}" font-size="12" fill="#8b949e">{lang}</text>
{icon(150, 116, ICON_STAR, "#8b949e")}
  <text x="172" y="128" font-family="{FONT}" font-size="12" fill="#8b949e">{stars}</text>
{icon(220, 116, ICON_FORK, "#8b949e")}
  <text x="242" y="128" font-family="{FONT}" font-size="12" fill="#8b949e">{forks}</text>
</svg>"""
